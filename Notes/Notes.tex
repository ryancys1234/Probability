...
\subsection*{Questions}
\begin{itemize}
    \item \textbf{3.1:} Theorem 3.2.8: ii) Since $f$ is continuous, $\forall \varepsilon > 0, \exists \delta > 0$ s.t. $|X_n - X| \leq \delta \implies |f(X_n) - f(X)| \leq \varepsilon$. By assumption, $1 = \lim \limits_{n \to \infty} \p(|X_n - X| \leq \delta) \leq \lim \limits_{n \to \infty} \p(|f(X_n) - f(X)| \leq \varepsilon)$.
